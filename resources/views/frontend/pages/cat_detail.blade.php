...
@section('meta')
    ...
    <meta property="og:title" content="VuFood - {{ $cat_news->name }}"/>
    ...
    <div class="container">
        <div class="row">
            <div class="col-xl-8 col-lg-8 col-md-12 news mb-5">
                <div class="row">
                    <h3 class="mb-5"><span style="text-transform: uppercase">{{ $cat_news->name }}</span></h3>
                    @if(!empty($news))
                        @foreach($news as $new)
            ...
            <div class="col-xl-4 col-lg-4 col-md-12 events">
                <div class="row">
                    <h3 class="mb-5">TIN MỚI NHẤT</h3>
                    @if(!empty($news_lastest))
                        @foreach($news_lastest as $news_l)
                            @php($link = route('article',['cat_slug' => $news_l->category->slug, 'art_slug' => $news_l->slug]))
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-xl-9 col-lg-9 col-md-11 col-10 text-event">
                                        <h4 class="title-event">
                                            <a href="{{ $link }}">{{ $news_l->title }}</a>
                                        </h4>
                                        <p class="date-event">
                                            <span>{{ date('d/m/Y',strtotime($news_l->date)) }}</span>
                                        </p>
                                    </div>
                                    <div class="bd-bot"></div>
                                </div>
